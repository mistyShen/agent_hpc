...
BACKEND_REGISTRY: tuple[BackendSpec, ...] = (
    ...
    _backend(
        ...
        "optional_backend",
        "publication",
        "Pseudobulk DESeq2/edgeR differential expression",
        "DESeq2 + edgeR",
        ("pseudobulk_counts", "pseudobulk_design"),
        "ultimate-rnaseq",
        "slurm/scrna_pseudobulk_de.sbatch",
        ("pseudobulk_de_results.tsv", "pseudobulk_de_backend_status.tsv", "pseudobulk_de_backend_manifest.json", "pseudobulk_de_volcano.png", "scrna_pseudobulk_de_backend.rds"),
        "Slurm validated synthetic replicate pseudobulk count/design matrix",
        ("必须使用 raw counts 和 biological replicate-aware design；cluster-level pseudobulk 不是 cell-level DE。",),
        production_allowed=True,
        backend_status="fully_automatic_validated_entrypoint",
        skip_reason="",
        resource_profile="medium_r",
        interpretation_warning="pseudobulk DE 是样本层统计结果，不能当作单细胞逐细胞独立检验。",
    ),
    _backend(
    ...
    _backend(
        ...
        ("velocity_graph.tsv", "velocity_embedding.tsv", "velocity_summary.tsv", "velocity_backend_manifest.json", "velocity_embedding.png", "scrna_velocity.h5ad"),
        "scVelo pancreas public h5ad with spliced/unspliced layers",
    ...
    _backend(
        ...
        "ultimate-scrna-r",
        "slurm/cite_seq_dsb_validation.sbatch",
        ("dsb_normalized_matrix.tsv", "background_summary.tsv", "dsb_backend_status.tsv", "dsb_backend_manifest.json", "dsb_heatmap.png", "cite_seq_dsb_backend.rds"),
        "dsb package CITE-seq cell and empty-droplet example data",
        ("需要 isotype/empty/background 信息；缺失时不能伪装完成 DSB。",),
        production_allowed=True,
        backend_status="fully_automatic_validated_entrypoint",
        skip_reason="",
        resource_profile="medium_r",
        interpretation_warning="DSB 是 ADT 背景校正，不是全蛋白组定量；必须记录 empty/background droplets 和 isotype controls。",
    ),
    _backend(
    ...
    _backend(
        ...
        "pandas matrix QC + Signac/ArchR/minfi handoff",
        ("scbs_output", "cuttag_peak_matrix", "cutrun_peak_matrix", "bed_region_table", "region_matrix", "h5ad"),
        "ultimate-scatac-multiome",
        "slurm/scepi_backend_validation.sbatch",
        ("feature_qc.tsv", "sample_qc.tsv", "missing_value_summary.tsv", "differential_region_handoff.tsv", "region_heatmap.png"),
