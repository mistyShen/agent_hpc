...
BACKEND_REGISTRY: tuple[BackendSpec, ...] = (
    _backend(
        ...
        ("当前 Python MVP 可自动出 QC/图/设计表；正式 DESeq2/edgeR 后端仍需单独启用。",),
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Annotation backend not yet wired into unified run.",
        resource_profile="medium_scrna",
    ),
    _backend(
        "scrna.qc.scrublet",
    ...
    _backend(
        ...
        "NSCLC internal h5ad + PBMC3k public h5ad",
        ("signature score 不是功能实验；gene set 来源和 overlap 必须记录。",),
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Currently available as functional_state module outputs/handoff, not unified scrna backend.",
    ),
    _backend(
        ...
        "ultimate-scrna-communication",
        "slurm/scrna_communication.sbatch",
        ("liana_interactions.tsv", "communication_network.tsv", "communication_dotplot.png"),
        "NSCLC internal annotated h5ad",
        ("配体-受体结果是统计推断，不是直接机制证明。",),
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="LIANA kept optional due dependency conflict risk; backend wrapper not implemented.",
    ...
    _backend(
        ...
        ("cnv_inference_summary.tsv", "malignant_cell_candidates.tsv", "cnv_heatmap.png"),
        "NSCLC raw-count internal h5ad",
        ("transcriptome-inferred CNV 不是 DNA-level CNV；需要原始 count 和基因位置。",),
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Tumor validation script can call optional tools, but unified automatic preset is not complete.",
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Current validation is module script/handoff; unified backend runner not complete.",
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Public validation script exists; unified h5mu backend runner not complete.",
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Visium validation script exists; unified spatial backend runner not complete.",
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Public validation script exists; unified CITE backend runner not complete.",
    ...
    _backend(
        ...
        "NSCLC internal h5ad derived signature validation",
    ...
    _backend(
        ...
        "public MaxQuant-like table required",
        ("当前蛋白组仍是弱模块，需要真实公开/内部 validation 后才能超过 smoke。",),
        production_allowed=True,
        backend_status="fully_automatic_mvp",
        skip_reason="MVP exists but validation evidence still required for V3 release.",
        resource_profile="small_matrix",
    ),
    _backend(
    ...
    _backend(
        ...
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Matrix summary validation exists; R HTODemux backend not wired.",
        resource_profile="small_matrix",
    ),
    _backend(
    ...
    _backend(
        ...
        ("figure_index.tsv", "table_index.tsv", "sensitive_metadata_scan.tsv", "delivery_manifest.json"),
        "NSCLC h5ad internal validation",
        ("交互式浏览器只是展示；公开交付前必须脱敏 metadata。",),
        production_allowed=True,
        backend_status="planned_fully_automatic",
        skip_reason="Static delivery manifest exists; cellxgene/Vitessce packaging backend not complete.",
